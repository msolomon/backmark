...
var ChromePromisifier, addToZip, backupUrl, chromePromisify, cleanFilename, closeZipWriter, combineZipFiles, downloadPage, downloadPageWithAccept, eraseStorage, fetchContent, getBlob, getBookmarks, getBookmarksInfo, getConcurrency, getFailedAtDate, getFailedAtTimestamp, getFailedAtTimestamps, getInfo, getOneInfo, getPage, getPrefixed, getRetryDays, getSavedAtDate, getSavedAtDates, getSavedAtTimestamp, getSavedAtTimestamps, getSingle, getSinglePrefixed, linearizeTree, makeFilename, mkDataUriZipReader, mkEmptyZip, mkFullBundle, mkKeepAlive, mkTabLoaded, readBlob, recordFailed, recordSaved, removeEntry, removeExactUrlFromHistory, removeUrlFromHistory, runBackup, runMissingBackup, savePage, saveSingle, setConcurrency, setRetryDays, zipPage;
...
zipPage = function(url, blob, extension) {
  if (extension == null) {
    extension = 'mhtml';
  }
  return mkEmptyZip().then(function(zw) {
    return addToZip(zw, makeFilename(cleanFilename(url), extension), new zip.BlobReader(blob));
  }).then(closeZipWriter);
};
...
mkFullBundle = function(urlsToDataUri) {
  return mkEmptyZip().then(function(base) {
...
downloadPageWithAccept = function(url) {
  return chrome.tabs.createAsync({
    url: 'acceptDanger.html',
    active: true,
    selected: true
  }).then(function(tab) {
    return chrome.runtime.sendMessageAsync(null, {
      msg: 'download',
      tabId: tab.id,
...
backupUrl = function(url) {
  ...
  return chrome.tabs.createAsync({
    ...
    return _.forEach(results, function(result) {
      var extension, saving;
      saving = (function() {
        switch (result.msg) {
          case 'saveMHTML':
            return chrome.pageCapture.saveAsMHTMLAsync({
              ...
              return zipPage(url, blob);
            });
          case 'saveEmbed':
            extension = /-(\w+)$/.exec(cleanFilename(url))[1];
            return getBlob(result.url).then(function(blob) {
              return zipPage(url, blob, extension);
            });
        }
      })();
      return saving.then(function(zip) {
